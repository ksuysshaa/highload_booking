...
| **Бронирования и оплата**                | Создание/изменение заказа, обработка платежа        | **Транзакционный сервис с репликой БД**                | Критичная консистентность, требуют синхронную репликацию (Master-Master / Master-Standby)                                                       |
| **Отзывы, сообщения**                    | Чат, написание/чтение отзывов                       | **L7-балансировка, очереди / Async**                   | Меньший объём, но постоянная активность (перед заездом, после выезда). Может работать частично асинхронно                                       |
| **Регистрация / авторизация**            | Вход, создание учётной записи                       | **Сервис аутентификации**                              |                                                                            |
| **Управление объявлениями** (host-кабинет)| Загрузка фото, редактирование описаний, календарь   | **Отдельные микросервисы, L7-балансировка**            | Обычно нагружает системы хранения (при загрузке большого числа фотографий), но RPS относительно невысок                                         |
| **Фоновые задачи, аналитика**            | Отчёты, бэкапы, статистика, генерация рекомендаций  | **Отдельный контур (Kafka, RabbitMQ), асинхронная БД** | Запускаются «в фоне», не влияют напрямую на пользовательский SLA, но требуют больших мощностей и хранения больших объёмов данных               |
