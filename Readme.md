...
| Алгоритм                                                                                             | Область применения                | Описание                                                                                                                                                                                                                                                                                                       | Почему он                                                                                                                                                                                              |
|------------------------------------------------------------------------------------------------------|-----------------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **Асинхронное обновление рейтингов и отзывов через очередь сообщений (Kafka, Redis, CronJob)**        | Рейтинги и отзывы отелей          | 1. Новые оценки и отзывы сохраняются в Redis в режиме реального времени.<br>2. Каждые 5 минут CronJob извлекает накопленные данные, отправляет их в Kafka для агрегирования.<br>3. Обработанные данные обновляют рейтинги и кэш в MongoDB.                                                       | Обеспечивает высокую актуальность информации, минимизируя нагрузку на основную систему в условиях большого потока отзывов и оценок от миллионов пользователей.                                                 |
| **FIFO-кэширование поисковых запросов**                                                              | Управление кэшем поисковых запросов | Результаты поисковых запросов сохраняются в Redis с установленным TTL. При достижении лимита кэша старые записи удаляются в порядке FIFO, независимо от частоты использования, что гарантирует своевременное обновление данных о доступных объектах.                                               | Гарантирует, что результаты поиска остаются актуальными – даже если популярные объекты долго находятся в кэше, новые запросы всё равно получают свежую информацию о доступности и актуальных предложениях.         |
| **Рекомендательная система (ElasticSearch + гибридный подход)**                                       | Персональные рекомендации         | 1. **Сбор данных:** Из различных источников (поисковые запросы, просмотры, бронирования, отзывы) агрегируются данные о действиях пользователей и характеристиках объектов размещения, хранящиеся в MongoDB и индексированные в ElasticSearch.<br>2. **Анализ данных по профилю пользователя:** Анализируется история взаимодействий, формируется детальный профиль с учетом явных предпочтений (например, выбранные фильтры) и скрытых сигналов (время просмотра, последовательность действий).<br>3. **Контентная фильтрация:** Подбираются объекты, схожие с ранее интересными пользователю, исходя из характеристик (категория, местоположение, ценовой диапазон, удобства).<br>4. **Коллаборативная фильтрация:** Проводится анализ поведения пользователей с похожими предпочтениями для выявления объектов, которые могут быть интересны, но не попадали в явный запрос.<br>5. **Гибридный подход:** Результаты обоих методов объединяются, что позволяет компенсировать недостатки каждого подхода по отдельности.<br>6. **Интеграция с ElasticSearch:** ElasticSearch обеспечивает оперативный поиск и фильтрацию по множеству параметров, что позволяет быстро получать актуальные данные об объектах и моментально обновлять рекомендации в зависимости от изменений (например, обновление доступности, изменения цен).  | Персонализированные рекомендации улучшают пользовательский опыт за счёт быстрого доступа к релевантным и актуальным предложениям. Это способствует увеличению вовлеченности, росту числа бронирований и повышению общей конверсии сервиса.                         |
| **Асинхронная обработка бронирований через очередь сообщений (Kafka/RabbitMQ)**                       | Бронирование отелей               | Запрос на бронирование сначала помещается в очередь сообщений, где проходит проверку доступности объекта, инициируется транзакция бронирования и запускается процесс оплаты. При возникновении ошибок реализована повторная обработка или откат транзакции для обеспечения целостности данных. | Обеспечивает масштабируемость и отказоустойчивость системы при пиковых нагрузках, минимизируя риск ошибок, таких как двойное бронирование, и гарантируя стабильность процесса подтверждения бронирований.     |
